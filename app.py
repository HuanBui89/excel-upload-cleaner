...
if uploaded_files:
        ...
        try:
            ...
            for sheet in sheets:
                ...
                df["Ghi chú thêm"] = (
                    ...
                    " - KHÁCH KHÔNG NHẬN THU 30K, GỌI VỀ SHOP KHI ĐƯƠN SAI THÔNG TIN"
                )
    ...
    if all_data:
        ...
        if template_option == "Mẫu 2 - Chị Linh":
            ...
            product_counter_normal = defaultdict(int)
            product_counter_lonxon = defaultdict(int)
                ...
                if ext == "xlsx":
                    try:
                        ...
                        for sheet_name in xls.sheet_names:
                            if "lộn xộn" in sheet_name.lower():
                                sheet_lon_xon.add(sheet_name.strip())
                    except:
                        continue
            ...
            for idx in range(len(final)):
                ...
                sheet_name = final.loc[idx, "sheet_name"] if "sheet_name" in final.columns else ""

                if sheet_name in sheet_lon_xon:
                    product_counter_lonxon[ten_sp_goc] += 1
                    stt = product_counter_lonxon[ten_sp_goc]
                    ma_don_rieng = f"LỘN XỘN D.{day}.{month}.{stt}"
                    ghi_chu = f"{ma_don_rieng} [{ten_sp_goc} {size_goc}] - KHÁCH KHÔNG NHẬN THU 30K, GỌI VỀ SHOP KHI ĐƯƠN SAI THÔNG TIN"
                else:
                    ten_sp_rut_gon = re.sub(r'^\s*\d+[A-Z]*\s+', '', ten_sp_goc)
                    product_counter_normal[ten_sp_rut_gon] += 1
                    stt = product_counter_normal[ten_sp_rut_gon]
